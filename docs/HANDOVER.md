...
- **규모**: 월 5건 기준, 1인 운영
- **운영 위치**: 운영자 본인의 Mac에서 launchd로 상시 가동, ngrok 정적 도메인으로 외부 공개
...
```
  ...
  launchd ────▶ scripts/server.sh
                  ├─ llama-server  (+ watchdog 60s)
                  ├─ uvicorn (FastAPI)
                  └─ ngrok          (+ watchdog 60s)
...
**구성 컴포넌트**
- FastAPI + Jinja2 — 어드민 UI/라우트 (`main.py`, `templates/`)
...
- ngrok — 외부 접근. 무료 플랜 + 정적 도메인 1개. 2시간마다 끊김을 watchdog가 자동 복구
...
mkdir -p models
# Qwen2.5-7B-Instruct Q4_K_M (분할 2개)를 models/에 배치
# 약 4.3GB
...
기동 후 `http://127.0.0.1:8000` 또는 `https://{NGROK_DOMAIN}` 으로 접근.
...
| ngrok 정적 도메인 | `.env` `NGROK_DOMAIN` | dashboard.ngrok.com/domains (계정당 무료 1개) |
| 회사/디렉터 정보 | `.env` fallback + `/settings` DB | settings 테이블 우선, .env 폴백 |
| Slack Webhook | `.env` `SLACK_WEBHOOK` | **보류** — 워크스페이스 앱 한도 이슈 (8번 참고) |
| launchd plist | `~/Library/LaunchAgents/com.antiegg.b2b.plist` | git 비추적. 절대 경로 들어있음 — 머신 이전 시 수정 필요 |
...
| 매일 03:50 | daily_log_rotate | app/llama/ngrok 로그 회전 (1MB 이상만, 14일 보관) |
| 매일 09:00 / 18:00 | daily_reminder | 미발송 회신 초안 Slack 재알림 |
| 매일 09:00 | check_no_response / check_closed_lost | KNOCK 또는 CLOSED_LOST 전환 |

### 5.4 DB 스키마 요약

- **deals** — 딜 1건. company/contact/email/stage/summary + reply/knock draft + cond_* (계약 조건) + trigger_* (트리거 상태) + sign_token (전자서명 1회용)
...
- ngrok 도메인이 살아있는지 (https URL로 접속 한 번)

**수동 트리거**
- 백업 즉시 실행: `/settings` 페이지 "지금 백업" 버튼 (또는 `POST /admin/backup`)
- few-shot 사례 즉시 수집: topbar "사례 갱신" 버튼 — **CPU 추론이라 3~5분 끊김 주의**
...
### ngrok URL이 안 열림
1. watchdog가 60초마다 체크하지만 무료 플랜 2시간 끊김은 최대 ~75초 다운타임 발생 가능
2. 1분 기다려도 안 되면 `/usr/sbin/lsof -nP -iTCP:4040` — 프로세스 살아있는지
3. ngrok 계정의 정적 도메인이 만료/회수됐는지 dashboard.ngrok.com 확인
...
launchctl load ~/Library/LaunchAgents/com.antiegg.b2b.plist
```
`main.py`의 라우트는 프로세스 시작 시점에 픽스된다. Jinja 템플릿은 디스크에서 hot-reload 되지만, 신규 라우트는 reload 없이는 404로 나오고 새 템플릿만 보이는 mixed-signal 상태가 된다. llama 콜드 로딩 ~45초.
...
결정되면 `app/integrations/slack.py`의 통합 함수에 연결.
...
- 운영 환경: macOS 13 Ventura + M2, Python 3.9, Homebrew llama.cpp
